            ...
            @foreach ($machinesGlaceSeche as $machine)
                <li>
                    <div class="carousel">
                                ...
                                @foreach ($machine->images as $index => $image)
                                    <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                                        <img src="{{ asset($image->image_path) }}"
                                            alt="Image of {{ $machine->marque }} {{ $machine->modele }}">
                                    </div>
                                @endforeach
                            </div>
                            <a class="carousel-control-prev" href="#carousel-{{ $machine->id }}" role="button"
                                data-slide="prev">
                                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                <span class="sr-only">Previous</span>
                            </a>
                            <a class="carousel-control-next" href="#carousel-{{ $machine->id }}" role="button"
                                data-slide="next">
                        ...
                        <a href="{{ route('equipement.show', ['type' => 'machine-glace-seche', 'id' => $machine->id]) }}"
                            class="marque-modele">
                        ...
                        <ul>
                            @if ($machine->largeur_plateau_nettoyage)
                                <li><strong>Largeur du plateau de nettoyage:</strong>
                                    {{ $machine->largeur_plateau_nettoyage }}</li>
                            @endif
