            ...
            @foreach ($polisseusesPropane as $polisseuse)
                <li>
                    <div class="carousel">
                                ...
                                @foreach ($polisseuse->images as $index => $image)
                                    <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                                        <img src="{{ asset($image->image_path) }}"
                                            alt="Image of {{ $polisseuse->marque }} {{ $polisseuse->modele }}">
                                    </div>
                                @endforeach
                            </div>
                            <a class="carousel-control-prev" href="#carousel-{{ $polisseuse->id }}" role="button"
                                data-slide="prev">
                                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                <span class="sr-only">Previous</span>
                            </a>
                            <a class="carousel-control-next" href="#carousel-{{ $polisseuse->id }}" role="button"
                                data-slide="next">
                        ...
                        <a href="{{ route('equipement.show', ['type' => 'polisseuse-propane', 'id' => $polisseuse->id]) }}"
                            class="marque-modele">
                            <p class="marque-modele">{{ $polisseuse->marque }} {{ $polisseuse->modele }}</p>
                        </a>
                        <ul>
                            @if ($polisseuse->largeur_plateau_nettoyage)
                                <li><strong>Largeur du plateau de nettoyage:</strong>
                                    {{ $polisseuse->largeur_plateau_nettoyage }}</li>
                            @endif
                            ...
                            @if ($polisseuse->superficie_nettoyage)
                                <li><strong>Superficie de nettoyage:</strong> {{ $polisseuse->superficie_nettoyage }}
                                </li>
                            @endif
                            ...
                            @if ($polisseuse->documents->isNotEmpty())
                                ...
                                <ul>
                                    @foreach ($polisseuse->documents as $document)
                                        <li>
                                            <a href="{{ asset($document->document_path) }}" target="_blank">Consulter
                                                le document</a>
                                        </li>
                                    @endforeach
                                </ul>
